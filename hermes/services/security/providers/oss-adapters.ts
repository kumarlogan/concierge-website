...
export function makeConfigReviewExecutor(): CapabilityExecutor {
  return async (capability: string, args: Record<string, unknown>) => {
    ...
    return {
      ok: true,
      data: { tool: "sim", simulated: true, findings: [] as SecurityFinding[] },
      backend: "sim:config",
    };
  };
}
...
export function makeSimulatedSecurityExecutor(): CapabilityExecutor {
  ...
  return async (capability: string, args: Record<string, unknown>, ctx: { actor: string; env: string; approvalToken?: string }) => {
